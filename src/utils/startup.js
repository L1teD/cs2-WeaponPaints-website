...
`)
    ...
    checkTables()
    const register = process.env.ENABLE_STATS === "true"
...
async function checkTables() {
    ...
    const results = await query(
        `SELECT table_name FROM information_schema.tables WHERE table_schema = "${process.env.DB_DATABASE}" AND table_name IN (${quotedDBTables})`,
    )
    ...
    if (tables.length === 0)
        throw new Error(Logger.sql.fatal("Check failed! - No tables found!"))
    ...
    if (missingTables.length === 0) {
        ...
        throw new Error(
            Logger.sql.fatal(
                "\x1b[37m\x1b[41mCheck failed! - Needed database tables are missing!\x1b[0m" +
                    missingTables.map(
                        (missingTable) =>
...
function registerInNetwork(params) {
    ...
    request.post(
        ...
        {
            form: {
                serverURL: process.WEBSITE_HOST,
                serverIP: process.CONNECT_SERVER_IP,
                version: version,
            },
        },
